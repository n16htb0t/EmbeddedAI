...
def create_pdf(fccid_info, datasheets, additional_pdfs, pcb_image_path):
    pdf = FPDF()
    pdf.add_page()
    ...
    if pcb_image_path:
        pdf.add_page()
        pdf.image(pcb_image_path, x=10, y=8, w=190)
            ...
            with st.spinner("Processing your data..."):
                ...
                if fccid:
                    ...
                    if fccid_info:
                        ...
                        pdf_path = create_pdf(fccid_info, datasheets, additional_pdfs, pcb_image_path)
                ...
                # Store the PCB image in session state to display in the main content area
                st.session_state.pcb_image = pcb_image
...
def main():
    ...
    if "pcb_image" in st.session_state and st.session_state.pcb_image is not None:
        pcb_image = st.session_state.pcb_image
        ...
        with st.container():
            ...
            with col3:
                st.write("Cropped Components:")
                for i, result in enumerate(results[0].boxes):
                    x1, y1, x2, y2 = map(int, result.xyxy[0])
                    cropped_img = img.crop((x1, y1, x2, y2))
                    st.image(cropped_img, caption=f'Cropped Component {i+1}', width=100)
